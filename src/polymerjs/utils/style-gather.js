  ...
  if (m && m._cssText === undefined) {
    ...
    if (t) {
      cssText += cssFromTemplate(
          t, /** @type {templateWithAssetPath }*/ (m).assetpath);
    }
...
export function cssFromTemplate(template, baseURI) {
  ...
  for (let i = 0; i < e$.length; i++) {
    ...
    cssText += baseURI ? resolveCss(e.textContent, baseURI) : e.textContent;
  }
  return cssText;
}
...
export function cssFromModuleImports(moduleId) {
  ...
  for (let i = 0; i < p$.length; i++) {
    ...
    if (p.import) {
      ...
      cssText += resolveCss(container.textContent, importDoc.baseURI);
    }
  }
  return cssText;
}
