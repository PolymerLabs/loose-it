 ...
 * @private
 */
export function finalizeProperties(proto, properties) {
...
export const ElementMixin = dedupingMixin(base => {
   ...
   * (2) properties effects and observers are created from it at `finalize`
   * time.
   *
      ...
      if (superCtor.prototype instanceof PolymerElement) {
        klass.__classProperties = Object.assign(
            Object.create(propertiesForClass(
                /** @type PolymerElementConstructor */ (superCtor))),
            klass.__classProperties);
      }
    }
  ...
  /**
   * Returns true if a `klass` has finalized. Called in
   * `ElementClass.finalize()`
   * @param {PolymerElementConstructor} klass Element class
  ...
  function hasClassFinalized(klass) {
    return klass.hasOwnProperty(
        JSCompiler_renameProperty('__finalized', klass));
  }
  ...
  function finalizeClass(klass) {
    ...
    let proto = /** @type PolymerElementConstructor */ (klass).prototype;
    if (klass.hasOwnProperty(JSCompiler_renameProperty('is', klass)) &&
        klass.is) {
      register(proto);
    }
  ...
  function finalizeTemplate(proto, template, baseURI, is, ext) {
    ...
    let cssText = cssFromTemplate(template, baseURI) + cssFromModuleImports(is);
    if (cssText) {
  ...
  class PolymerElement extends polymerElementBase {
    ...
    /**
     * Returns the template that will be stamped into this element's shadow
     * root.
     *
    ...
    static get template() {
      if (!this.hasOwnProperty(JSCompiler_renameProperty('_template', this))) {
        this._template = DomModule &&
                DomModule.import(
                    /** @type PolymerElementConstructor*/ (this).is,
                    'template') ||
            ...
            Object
                .getPrototypeOf(
                    /** @type PolymerElementConstructor*/ (this).prototype)
    ...
    _initializeProperties() {
      ...
      if (this._template && !this._template.__polymerFinalized) {
        ...
        finalizeTemplate(
            /** @type {!PolymerElement} */ (this.__proto__),
            this._template,
            baseURI,
      ...
      for (let p in p$) {
        ...
        if (!this.hasOwnProperty(p)) {
          let value = typeof info.value == 'function' ? info.value.call(this) :
                                                        info.value;
    ...
    updateStyles(properties) {
      if (window.ShadyCSS) {
        window.ShadyCSS.styleSubtree(
            /** @type {!HTMLElement} */ (this), properties);
      }
    }
    ...
    static _parseTemplateContent(template, templateInfo, nodeInfo) {
      templateInfo.dynamicFns =
          templateInfo.dynamicFns || propertiesForClass(this);
