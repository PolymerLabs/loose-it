...
/**
 * All states that the parser can be in. The states represent the state-machine
 * as a whole.
 * @enum {number}
 */
const STATE =
  ...
  if (name) {
    if (name === 'true' || name === 'false') {
      bindingData.signature.args.push(
          {name, value: name == 'true', literal: true});
    } else {
      const arg = {name} arg.structured = isPath(name);
      if (arg.structured) {
...
function storeMethodNumber(bindingData, text, i) {
  ...
  bindingData.signature.args.push(
      {name: value, value: Number(value), literal: true});
}
  ...
  for (i = 0, l = text.length; i < l; i++) {
    ...
    switch (state) {
      ...
      case STATE.COLONNOTIFYEVENTFIRSTCLOSINGBINDING: {
        if (char === BINDINGS[bindingData.mode]) {
          bindingData.event =
              text.substring(bindingData.startCharAfterColon, i - 1).trim();
          const prop = text.substring(
                               bindingData.startChar,
                               bindingData.startCharAfterColon - 2)
                           .trim();
      ...
      case STATE.STRINGARG: {
        if (char === '\\') {
          ...
          const value = text.substring(bindingData.startChar, i)
                            ...
                            .replace(/&comma;/g, ',')
                            // repair extra escape sequences; note only commas
                            // strictly need escaping, but we allow any other
                            // char to be escaped since its likely users will do
                            // this
                            .replace(/\\(.)/g, '\$1');
          bindingData.signature.args.push({value, name: value, literal: true});
          bindingData.startChar = i + 1;
          state = STATE.METHOD;
        } else {
          escaped = false;
      ...
      case STATE.METHODCLOSED: {
        if (char === BINDINGS[bindingData.mode]) {
          ...
          console.warn(`Expected two closing "${
                                                BINDINGS[bindingData.mode]
                                              }" for binding "${text}"`);
        }
        break;
      }
      case STATE.METHODCLOSEDBINDING: {
        if (char === BINDINGS[bindingData.mode]) {
          ...
          console.warn(`Expected one closing "${
                                                BINDINGS[bindingData.mode]
                                              }" for binding "${text}"`);
        }
        break;
